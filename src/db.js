...
CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, message_id TEXT NOT NULL UNIQUE, source_group_id INTEGER REFERENCES groups(id) ON DELETE SET NULL, source_remote_id TEXT NOT NULL, event_json TEXT NOT NULL, text_content TEXT DEFAULT '', original_url TEXT DEFAULT '', platform TEXT DEFAULT '', status TEXT NOT NULL DEFAULT 'queued', product_title TEXT DEFAULT '', shopee_url TEXT DEFAULT '', confidence REAL, error TEXT DEFAULT '', received_at TEXT NOT NULL, processed_at TEXT);
